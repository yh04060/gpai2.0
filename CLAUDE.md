...
- **js 18개는 하나의 전역 스코프를 공유하는 클래식 스크립트**입니다 (모듈 아님).
...
6. **기밀 문구를 삭제하지 않습니다.** v7.11부터 화면 하단 고정 바가 아니라 「Q의 의견」
   드로어 최상단의 고정 공지(`.note-notice`)로 노출됩니다 — 항상 보이던 것을 드로어를
   열어야 보이는 것으로 낮춘 건 Q의 결정입니다(하단 바가 모든 화면을 가려 거슬린다는
   이유). 위치나 표현을 바꾸는 건 Q와 상의하되, 문구 자체를 없애지는 마세요
...
1. **스크립트 태그 순서 = 전역 정의 순서.** js 18개는 하나의 전역 스코프입니다.
...
3. **하단 중앙 「Q의 의견」 버튼과의 겹침.** v7.11부터 이 버튼이 화면 하단 중앙에
   ...
   같이 조정하세요. 우하단은 채팅 FAB(`right:22px` · `bottom:22px`) 자리입니다.
   겹침 판정은 눈보다 `getBoundingClientRect()` 스크립트가 확실합니다
...
### 브랜치 — `main`에서 직접 작업합니다 (2026-09-01 변경)
...
**왜 작업 브랜치를 안 쓰나** — 작업 브랜치의 존재 이유는 보통 ①여러 명이 같은 코드를
건드릴 때의 충돌 방지 ②CI가 검증을 마칠 때까지의 격리 ③리뷰어가 머지 전에 볼 시간인데,
이 저장소는 셋 다 해당이 없습니다 (1인 개발 · CI도 빌드도 없음 · 리뷰어가 Q 본인).

반대로 비용은 실재합니다. **Q는 Claude의 클라우드 컨테이너 안을 볼 수 없어서, 배포된
사이트가 작업 결과를 확인하는 유일한 경로입니다.** 그래서 브랜치를 끼우면 확인 사이클마다
`체크아웃 → 머지 → 푸시` 3단계가 순수 오버헤드로 붙고, "배포된 상태로 확인하면서 다음
커밋을 이어간다"는 실제 작업 리듬을 방해합니다. 정적 파일이라 무엇이 깨져도
`git revert` 한 번 + 30초면 원복된다는 점도 근거입니다.
...
- 클라우드 세션의 컨테이너는 임시입니다. **푸시하지 않은 커밋은 컨테이너 회수와 함께
  ...
  (Q가 "커밋만 해줘"라고 명시한 경우만 예외). 단 `main` 직접 작업이므로
  **푸시 전에 검증 루틴을 반드시 통과**시켜야 합니다 — 푸시가 곧 배포입니다
- 문서만 고친 커밋은 버전을 올리지 않습니다 (선례: `ce2b0f0`, `5230e88`).
  코드가 바뀔 때만 버전 로그 + 드로어 배지를 올립니다
