...
새 버전이 바로 안 보이면 브라우저 캐시 때문이니 **Ctrl+Shift+R**로 하드 리프레시하세요.
...
## 기밀 — 저장소가 Public이라는 전제

**이 저장소는 Public입니다.** 무료 플랜에서 Private 저장소에는 Pages를 켤 수 없어,
배포를 위해 Q가 선택한 것입니다. 따라서 다음이 전부 인터넷에 공개돼 있습니다:

- `index.html` · `css/` · `js/` 소스 전체
- 이 `CLAUDE.md` (제품 전략 · 3단 위계 설계 · 다음 이터레이션 계획)
- 「Q의 의견」 드로어 내용 (포지셔닝 · 설계 의도 · 받고 싶은 피드백)
- 배포된 사이트 자체

### 그래서 커밋하면 안 되는 것

프로토타입의 모든 데이터는 **데모**여야 합니다. 다음은 절대 넣지 마세요:

- 실제 학생·사용자의 개인정보, 실제 성적·과제 내용
- API 키·토큰·인증정보 (이 프로토타입엔 백엔드가 없으므로 원래 필요 없습니다)
- 튜링의 비공개 사업 수치, 미공개 로드맵, 대외비 계약·파트너 정보

**Git 히스토리는 지워지지 않습니다.** 한 번 푸시하면 이후 파일에서 삭제해도 과거 커밋에
그대로 남아 누구나 볼 수 있습니다. 커밋 전에 확인하는 것이 유일한 방어선입니다.
민감한 것이 올라갔다면 파일 수정으로 끝내지 말고 즉시 Q에게 알리세요.

### 하단 기밀 바는 유지합니다

저장소가 Public이 됐어도 **이 프로토타입 자체는 튜링 내부 검토용**이라는 사실은
변하지 않습니다. 모든 화면 하단의 문구를 임의로 바꾸거나 가리지 마세요:

**"튜링 회사 내부용이며, 외부로의 유출과 공유를 엄격히 금지합니다."**

이 바는 사이트를 보는 사람에게 "완성된 제품이 아니라 내부 검토용 목업"임을 알리는
역할을 합니다.
