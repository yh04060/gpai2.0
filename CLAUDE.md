...
| `01-assets-avatars.js` | **프로젝트 AI 프로필 캐릭터 20종** — 인라인 SVG 레지스트리 + 렌더러. 같은 01번대(자산)라 순서 무관 | `AVATARS` `avatarSVG` `avatarFor` `avatarDefault` |
...
이 저장소의 그래픽은 **인라인 SVG가 기본**입니다 — 빌드·의존성 없이 크기에 상관없이
선명하고, 색을 코드로 통제할 수 있고, 파일 요청이 0이라서입니다. 단, 이건 "규칙"이
아니라 "기본값"이고, 진짜 일러스트나 사진이 필요하면 파일을 씁니다.

**캐릭터·아바타 같은 반복 자산은 `js/01-assets-avatars.js` 패턴을 따르세요:**

- **데이터(레지스트리) + 렌더러 분리.** `AVATARS`는 `{k, n, bg, draw}` 항목의 배열이고,
  `avatarSVG(k,size)`가 그리는 유일한 함수입니다. 화면 코드는 키만 다룹니다
  (`p.avatar='owl'`). 그림을 바꿔도 화면 코드는 안 건드립니다
- **자체 완결 조각.** 각 캐릭터는 배경(둥근 사각)까지 포함한 64×64 SVG라, 컨테이너
  크기·모양과 무관하게 같은 모습입니다. 컨테이너는 `background:transparent`만 주면 됩니다
  (`.pj-pava.av` `.pj-mini.av`)
- **공통 파츠 헬퍼**(`avEyes` `avBlush` `avW` `avArc`)로 눈·볼·입을 통일해 20종이
  한 세트로 읽히게 합니다. 새 캐릭터를 추가할 때도 이 헬퍼를 쓰세요
- **기본값은 결정적.** `avatarDefault(name)`이 이름 해시로 캐릭터를 고르므로 "설정
  안 한" 에이전트도 항상 얼굴이 있고, 새로고침해도 바뀌지 않습니다. 선택은 옵션이지
  퍼널이 아닙니다
- **파일로 교체할 길을 열어둠.** 항목에 `draw` 대신 `src:'assets/avatars/owl.png'`을
  주면 `avatarSVG()`가 `<img>`로 렌더합니다. 나중에 디자이너 일러스트로 갈아끼울 때
  화면 코드 수정이 0입니다

**래스터(PNG·JPG)가 필요해지면** — `assets/<용도>/` 폴더를 만들고 파일명은 레지스트리
키와 같게(`owl.png`) 맞추세요. 알아둘 제약: **Claude 클라우드 세션은 이미지를 생성할
수도, 외부에서 내려받을 수도 없습니다**(프록시가 외부 HTTPS를 차단). 그래서 일러스트
파일은 Q 쪽에서 만들어 저장소에 커밋해야 하고, Claude는 그 파일을 레지스트리에 연결하는
쪽을 맡습니다. 이모지는 OS마다 생김새가 달라 프로필용으로는 쓰지 않습니다.
