...
| 브랜치 | 용도 |
|---|---|
| `main` | 기본 브랜치이자 **배포 브랜치** — GitHub Pages가 여기를 그대로 서빙합니다. 직접 커밋하지 않고, 병합 시점은 Q가 정합니다 |
| `claude/*` | 작업 브랜치. 개발은 항상 여기서 하고, Q 확인 후 `main`에 머지합니다 (`git branch -rv`로 확인) |
...
1. `git fetch origin` → `git branch -rv`로 **최신 작업 브랜치를 찾아 체크아웃**
   (`main`은 배포 브랜치라 직접 커밋하지 않습니다 — 세션에 지정된 `claude/*`
   브랜치가 있으면 거기서, 없으면 최신 작업 브랜치에서 작업)
